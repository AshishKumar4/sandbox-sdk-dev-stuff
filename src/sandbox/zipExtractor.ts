...
export class ZipExtractor {
    ...
    // Reusable TextDecoder (thread-safe, stateless)
    ...
    static extractFiles(zipBuffer: ArrayBuffer): TemplateFile[] {
        try {
            ...
            for (const [filePath, fileData] of Object.entries(unzipped)) {
                ...
                // Fast path: Check if file extension is known binary
                const isBinary = this.isBinaryExtension(filePath);
                
                if (isBinary) {
                    // Skip UTF-8 decode attempt, go straight to base64
                    fileContents = `base64:${this.fastBase64Encode(fileData)}`;
                } else {
    ...
    /**
     * Fast check if file extension is known binary type
     * Avoids expensive UTF-8 decode attempt
     */
    ...
    /**
     * Fast base64 encoding optimized for CPU efficiency
     * Avoids intermediate array and map operations
     */
    private static fastBase64Encode(data: Uint8Array): string {
        // Use native btoa with optimized binary string conversion
        let binaryString = '';
